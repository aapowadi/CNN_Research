...
def eval_pose_accuracy(t_gt, t_pr, q_gt, q_pr, path_and_file = ""):
    ...
    """
    t_mean, t_acc, cs_dist = eval_translation_accuracy(t_gt, t_pr)
    q_mean, q_acc = evaluate_orientation_accuracy(q_gt, q_pr, cs_dist)

    # write the results to a file
    write_results(t_acc, q_acc, path_and_file)

    return t_mean, t_acc, q_mean, q_acc


def eval_translation_accuracy(t_gt, t_pr):
    """
    ...
    for i in range(0,size):
        ...
        t_acc = ((gt[0] - pr[0] )/ gt[0] + (gt[1] - pr[1]) / gt[1] + (gt[2] - pr[2]) / gt[2]) / 3;
    ...
    return distance_error, all_acc, cs_dist


def evaluate_orientation_accuracy(q_gt, q_pr, cs_dist):
    """
    ...
    return orientation_error, all_errors
...
def write_results(t_array, q_array, file):
    ...
    for i in range(0, size):
        out = str(i) + "," + str(t_array[i]) + "," + str(q_array[i])  + "\n"
        file.write(out)
